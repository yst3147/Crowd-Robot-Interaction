...
def main():
    ...
    memory = ReplayMemory(100000000)         # sufficiently large to store expert demonstration
    ...
    if os.path.isfile(demonstration_file):
        memory.memory = torch.load(demonstration_file)
    ...
    episode = 0
    while episode < il_episodes:
    
        explorer.run_k_episodes(il_episodes // 10, 'train', update_memory=True, imitation_learning=True, noise_explore=noise_explore)
        episode += il_episodes // 10
